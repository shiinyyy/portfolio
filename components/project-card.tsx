  ...
  return (
    <Card className="bg-zinc-900/70 backdrop-blur-md border-black hover:border-white transition-colors overflow-hidden group h-full flex flex-col">
      <div className="aspect-video bg-zinc-950 relative overflow-hidden">
        <Image
          ...
          unoptimized={
            ...
            (currentImage.includes("drive.google.com") || currentImage.includes("blob.vercel-storage.com"))
          }
        />
      ...
      <CardHeader className="flex-none space-y-2 pb-3">
        <CardTitle className="line-clamp-2 min-h-[2rem] text-lg text-white">{project.title}</CardTitle>
        <CardDescription className="text-zinc-200 min-h-[8rem] line-clamp-4 text-base leading-relaxed">{project.description}</CardDescription>
      </CardHeader>
      <CardContent className="pt-0">
        <div className="flex flex-wrap gap-1.5">
          {project.technologies.map((tech, i) => (
            <span key={i} className="px-2 py-0.5 bg-zinc-300 rounded-md text-xs whitespace-nowrap">
              {tech}
            </span>
          ))}
        </div>
